...
/*------------------------------------------------------------------------
 * RuleDictCEntryList constructors
 */

RuleDictCEntryList::RuleDictCEntryList() : GROWTH_AMOUNT(4), NULL_INDEX(-1), MAX_GROWTH(32768)
{
   _list		= (RuleDictCEntry*)NULL;
   _count		= 0;
   _space		= 0;
...
RuleDictCEntryList::RuleDictCEntryList(unsigned ga) : GROWTH_AMOUNT(ga), NULL_INDEX(-1), MAX_GROWTH(ga)
{
   _list		= (RuleDictCEntry*)NULL;
   _count		= 0;
   _space		= 0;
...
RuleDictCEntryList::RuleDictCEntryList(const RuleDictCEntryList& l):GROWTH_AMOUNT(4), NULL_INDEX(-1), MAX_GROWTH(32768)
{
   _list		= (RuleDictCEntry*)NULL;
   _count		= 0;
   _space		= 0;
...
/*------------------------------------------------------------------------
 * RuleDictCEntryList destructor
 */

RuleDictCEntryList::~RuleDictCEntryList()
{
...
RuleDictCEntryList&
RuleDictCEntryList::operator+=(const RuleDictCEntryList& l)
{
   if ( this != &l ) {
      register int count = l.size();
      register int i;
      for (i=0; i<count; i++) {
	 RuleDictCEntry *item = l[i];
	 add(*item);
      }
   }
   return *this;
...
int
RuleDictCEntryList::includes(const RuleDictCEntry& obj) const
{
...
RuleDictCEntry*
RuleDictCEntryList::append(const RuleDictCEntry& obj)
{
   return insert(obj, _count);
}

RuleDictCEntry*
RuleDictCEntryList::prepend(const RuleDictCEntry& obj)
{
   return insert(obj, 0);
}

RuleDictCEntry*
RuleDictCEntryList::add(const RuleDictCEntry& obj)
{
   return append(obj);
}
...
void
RuleDictCEntryList::SetSorted(char val)
{
   if ( sorted != val ) {
      sorted = val;
      if ( sorted ) sort();
...
void	
RuleDictCEntryList::AllowDuplicates(char val)
{
   allow_duplicates = val;
}

void	
RuleDictCEntryList::AutoShrink(char val)
{
   autoShrink = val;
   if ( autoShrink ) shrink();
}
...
void	
RuleDictCEntryList::SetCapacity(unsigned count)
{
   grow(count*sizeof(RuleDictCEntry));
}
...
RuleDictCEntry*
RuleDictCEntryList::operator[](unsigned i) const
{
   RuleDictCEntry	*tp = (RuleDictCEntry*)NULL;
   if (i < _count)
      tp = &_list[i];
   else {
      cerr << "RuleDictCEntryList[" << i << "]: reference out of bounds.";
...
RuleDictCEntry*
RuleDictCEntryList::First() const
{
   RuleDictCEntry	*tp = (RuleDictCEntry*)NULL;
...
ostream&
RuleDictCEntryList::printOn(ostream& strm) const
{
...
void
RuleDictCEntryList::grow(unsigned newSize)
{
/* Increase space */
...
/* Allocate memory for new list */
   RuleDictCEntry	*old_list = _list;
   _list = new RuleDictCEntry[_space];
   if ( !_list ) {
...
void
RuleDictCEntryList::shrink()
{
   if ( !autoShrink ) return;

/* Decrease space if necessary */

   int  needed = _count * sizeof(RuleDictCEntry);
...
int
RuleDictCEntryList::indexOf(const RuleDictCEntry& obj) const
{
...
RuleDictCEntry*
RuleDictCEntryList::insert(const RuleDictCEntry& obj, unsigned i)
{
...
/* Otherwise, see if index is valid */
   if (i > _count) return (RuleDictCEntry*)NULL;

/* Don't allow same address in list twice */
   if ( !allow_duplicates && includes(obj) ) return (RuleDictCEntry*)NULL;
...
/* Make a space for the new entry by moving all others back */
   ...
   _count++;
   RuleDictCEntry	*tp = (RuleDictCEntry *)&obj;
...
void
RuleDictCEntryList::remove(unsigned index)
{
   if (index < _count) {
...
void
RuleDictCEntryList::remove(const RuleDictCEntry& obj)
{
/* Look up this entry */
...
RuleDictCEntry*
RuleDictCEntryList::before(const RuleDictCEntry& obj) const
{
/* Look up this entry */

   register int i = indexOf(obj);

   if ( i == NULL_INDEX || i == 0 ) return (RuleDictCEntry*)NULL;
   return &_list[i-1];
}
...
unsigned
RuleDictCEntryList::orderOf(const RuleDictCEntry& obj) const
{
/*
...
} /* End RuleDictCEntryList::orderOf */

/*-------------------------------------------------------------*/

int
RuleDictCEntryList::compare(const void *a, const void *b)
{
   RuleDictCEntry *ta = (RuleDictCEntry *)a;
   RuleDictCEntry *tb = (RuleDictCEntry *)b;
...
void
RuleDictCEntryList::sort()
{
   qsort((void *)_list, (size_t)_count, (size_t)sizeof(RuleDictCEntry),
...
void
RuleDictCEntryList::sort(int (*compar)(const void*, const void*))
{
   if ( !compar ) return;
   qsort((void *)_list, (size_t)_count, (size_t)sizeof(RuleDictCEntry), compar);
}
...
RuleDictC&
RuleDictC::operator=(const RuleDictC& d)
{
   if ( this != &d ) {
...
int
RuleDictC::indexOf(const RuleDictCEntry& entry) const
{
   return RuleDictCEntryList::indexOf(entry);
}

int
RuleDictC::includes(const RegexC& key) const
{
   return (indexOf(key) != NULL_INDEX);
}

int
RuleDictC::includes(const RuleDictCEntry& entry) const
{
   return RuleDictCEntryList::includes(entry);
}

/* Deleting entries */

void
RuleDictC::remove(unsigned index)
{
   RuleDictCEntryList::remove(index);
}

void
RuleDictC::remove(RuleDictCEntry& entry)
{
   RuleDictCEntryList::remove(entry);
}

/*-------------------------------------------------------------*/

int
RuleDictC::indexOf(const RegexC& key) const
{
...
StringC*
RuleDictC::definitionOf(const RegexC& key) const
{
...
RegexC&
RuleDictC::keyOf(const int index) const
{
...
StringC&
RuleDictC::valOf(const int index) const
{
...
RuleDictCEntry*
RuleDictC::entryOf(const RegexC& key) const
{
   int	i = indexOf(key); /* Get index of key */
   return ((i==NULL_INDEX) ? (RuleDictCEntry*)NULL : &_list[i]);
}

/*-------------------------------------------------------------*/

RuleDictCEntry*
RuleDictC::add(const RegexC& key, const StringC& val)
{
/* Check if this entry is already present */

   if ( includes(key) ) {
      if ( *definitionOf(key) != val ) {
         return (RuleDictCEntry*)NULL; /* Same key with different value */
      } else {
         return entryOf(key);
      }
   } else { /* Add it */
      RuleDictCEntry      ent(key, val);
      return append(ent);
   }
}
...
RuleDictCEntry*
RuleDictC::modify(const RegexC& key, const StringC& val)
{
   int	i = indexOf(key); /* Look up this entry */

   if ( i == NULL_INDEX ) {
      return (RuleDictCEntry*)NULL;
   } else {
...
RuleDictCEntry*
RuleDictC::modify(unsigned index, const StringC& val)
{
   if (index < _count) {
...
void
RuleDictC::remove( RegexC& key)
{
   int	i = indexOf(key); /* Look up this entry */

   if ( i != NULL_INDEX ) {
      RuleDictCEntryList::remove(i);
   }
}
