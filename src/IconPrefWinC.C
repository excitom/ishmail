/*
 *  $Id: IconPrefWinC.C,v 1.2 2000/05/07 12:26:11 fnevgeny Exp $
 *  
...
IconPrefWinC::IconPrefWinC(Widget parent) : DictEditWinC(parent, "iconWin")
{
...
void
IconPrefWinC::DoPopup(Widget, IconPrefWinC *This, XtPointer)
{
...
//
   ...
   IconPrefC	*prefs = ishApp->iconPrefs;
   unsigned	count = prefs->iconRules.size();
   for (int i=0; i<count; i++) {

      RegexC&	key = prefs->iconRules.keyOf(i);
      StringC&	val = prefs->iconRules.valOf(i);
...
//
// Delete any icons that weren't also added.
//
   ...
   for (i=0; i<count; i++) {
	 ...
	 prefs->iconRules.remove(tmp);
      }
   }

//
// Add any icons that weren't later deleted.
//
   count = addList.size();
   for (i=0; i<count; i++) {
	 ...
	 RegexC	tmp = item->Name();
	 prefs->iconRules.add(tmp, *(item->FieldList()[1]));
      }
   }

//
// Modify any icons that weren't added or deleted.
//
   count = modList.size();
   for (i=0; i<count; i++) {
	 ...
	 RegexC	tmp = item->Name();
	 prefs->iconRules.modify(tmp, *(item->FieldList()[1]));
      }
   }
...
//
   count = prefs->iconRules.size();
   for (i=0; i<count; i++) {

      RegexC&   dictName = prefs->iconRules.keyOf(i);
...
//
	 ...
	 RuleDictCEntry       entry = *prefs->iconRules.entryOf(itemName);
	 prefs->iconRules.remove(entry);
	 prefs->iconRules.insert(entry, i);
      }
...
int
IconPrefWinC::Compare(const void *a, const void *b)
{
   //cout <<"IconPrefWinC::Compare" <<endl;

   VItemC	*via = *(VItemC **)a;
   VItemC	*vib = *(VItemC **)b;

   IconPrefWinC	*This = (IconPrefWinC *)via->UserData();
