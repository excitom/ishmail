...
ViewListC::ViewListC() : GROWTH_AMOUNT(4), NULL_INDEX(-1), MAX_GROWTH(32768)
{
   _list		= (ViewC**)NULL;
   _count		= 0;
   _space		= 0;
...
ViewListC::ViewListC(unsigned ga) : GROWTH_AMOUNT(ga), NULL_INDEX(-1), MAX_GROWTH(ga)
{
   _list		= (ViewC**)NULL;
   _count		= 0;
   _space		= 0;
...
ViewListC::ViewListC(const ViewListC& l):GROWTH_AMOUNT(4), NULL_INDEX(-1), MAX_GROWTH(32768)
{
   _list		= (ViewC**)NULL;
   _count		= 0;
   _space		= 0;
...
ViewListC&
ViewListC::operator+=(const ViewListC& l)
{
   if ( this != &l ) {
...
unsigned
ViewListC::orderOf(const ViewC* op) const
{
   return orderOf(*op);
}
...
int
ViewListC::includes(const ViewC& obj) const
{
   return (indexOf(obj) != NULL_INDEX);
}

int
ViewListC::includes(const ViewC* op) const
{
...
ViewC*
ViewListC::append(const ViewC* op)
{
   return insert(op, _count);
}

ViewC*
ViewListC::prepend(const ViewC* op)
{
   return insert(op, 0); 
}

ViewC*
ViewListC::add(const ViewC* op)
{
   return append(op); 
}
...
void
ViewListC::AllowDuplicates(char val)
{
   allow_duplicates = val;
}

void
ViewListC::AutoShrink(char val)
{
   autoShrink = val;
   if ( autoShrink ) shrink();
}
...
void
ViewListC::SetCapacity(unsigned count)
{
   grow(count*sizeof(ViewC*));
}

ViewC*
ViewListC::operator[](unsigned i) const
{
   ViewC	*tp = (ViewC*)NULL;
   if (i < _count)
      tp = _list[i];
   else {
      cerr << "ViewListC[" << i << "]: reference out of bounds.";
...
ostream&
ViewListC::printOn(ostream& strm) const
{
...
void
ViewListC::grow(unsigned newSize)
{
/* Increase space */
...
/* Allocate memory for new list */
   ViewC	**old_list = _list;
   _list = new ViewC*[_space];
   if ( !_list ) {
...
/* Decrease space if necessary */
   ...
   int  needed = _count * sizeof(ViewC*);
...
int
ViewListC::indexOf(const ViewC& obj) const
{
...
int
ViewListC::indexOf(const ViewC* op) const
{
/* Loop through the list until the entry is found */
...
ViewC*
ViewListC::insert(const ViewC* op, unsigned i)
{
...
/* Otherwise, see if index is valid */
   else if (i > _count) return (ViewC*)NULL;
...
ViewC*
ViewListC::replace(const ViewC* op, unsigned i)
{
/* See if index is valid */
   if (i > _count) return (ViewC*)NULL;

/* Don't allow same address in list twice */
   if ( op != (ViewC*)NULL && !allow_duplicates ) {
      int	index = indexOf(op);
      if ( index != NULL_INDEX ) return _list[index];
   }
...
void
ViewListC::remove(unsigned index)
{
   if (index < _count) {
...
void
ViewListC::remove(const ViewC& obj)
{
/* Look up this entry */
...
void
ViewListC::remove(const ViewC* op)
{
/* Look up this entry */
...
void
ViewListC::removeNulls()
{
/* Collapse list */
   register int	j = 0;
   register int i;
   for (i=0; i<_count; i++) {
      if ( _list[i] != (ViewC*)NULL ) {
...
ViewC*
ViewListC::before(const ViewC* op) const
{
/* Look up this entry */

   register int i = indexOf(op);
   if ( i == NULL_INDEX || i == 0 ) return (ViewC*)NULL;
   return _list[i-1];
}
...
unsigned
ViewListC::orderOf(const ViewC& obj) const
{
/*
...
} /* End ViewListC::orderOf */

/*-------------------------------------------------------------*/

int
ViewListC::compare(const void *a, const void *b)
{
   ViewC	*ta = *(ViewC **)a;
   ViewC	*tb = *(ViewC **)b;
...
ViewListC::sort()
{
   qsort((void *)_list, (size_t)_count, (size_t)sizeof(ViewC*),
...
void
ViewListC::sort(int (*compar)(const void*, const void*))
{
   if ( !compar ) return;
   qsort((void *)_list, (size_t)_count, (size_t)sizeof(ViewC*), compar);
}
