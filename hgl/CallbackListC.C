...
CallbackListC::CallbackListC() : GROWTH_AMOUNT(4), NULL_INDEX(-1), MAX_GROWTH(32768)
{
   _list		= (CallbackC**)NULL;
   _count		= 0;
   _space		= 0;
...
CallbackListC::CallbackListC(unsigned ga) : GROWTH_AMOUNT(ga), NULL_INDEX(-1), MAX_GROWTH(ga)
{
   _list		= (CallbackC**)NULL;
   _count		= 0;
   _space		= 0;
...
CallbackListC::CallbackListC(const CallbackListC& l):GROWTH_AMOUNT(4), NULL_INDEX(-1), MAX_GROWTH(32768)
{
   _list		= (CallbackC**)NULL;
   _count		= 0;
   _space		= 0;
...
CallbackListC&
CallbackListC::operator+=(const CallbackListC& l)
{
   if ( this != &l ) {
...
unsigned
CallbackListC::orderOf(const CallbackC* op) const
{
   return orderOf(*op);
}
...
int
CallbackListC::includes(const CallbackC& obj) const
{
   return (indexOf(obj) != NULL_INDEX);
}

int
CallbackListC::includes(const CallbackC* op) const
{
...
CallbackC*
CallbackListC::append(const CallbackC* op)
{
   return insert(op, _count);
}

CallbackC*
CallbackListC::prepend(const CallbackC* op)
{
   return insert(op, 0); 
}

CallbackC*
CallbackListC::add(const CallbackC* op)
{
   return append(op); 
}
...
void
CallbackListC::SetSorted(char val)
{
   if ( sorted != val ) {
      sorted = val;
      if ( sorted ) sort();
...
void
CallbackListC::AllowDuplicates(char val)
{
   allow_duplicates = val;
}

void
CallbackListC::AutoShrink(char val)
{
   autoShrink = val;
   if ( autoShrink ) shrink();
}
...
void
CallbackListC::SetCapacity(unsigned count)
{
   grow(count*sizeof(CallbackC*));
}

CallbackC*
CallbackListC::operator[](unsigned i) const
{
   CallbackC	*tp = (CallbackC*)NULL;
   if (i < _count)
      tp = _list[i];
   else {
      cerr << "CallbackListC[" << i << "]: reference out of bounds.";
...
ostream&
CallbackListC::printOn(ostream& strm) const
{
...
void
CallbackListC::grow(unsigned newSize)
{
/* Increase space */
...
/* Allocate memory for new list */
   CallbackC	**old_list = _list;
   _list = new CallbackC*[_space];
   if ( !_list ) {
...
void
CallbackListC::shrink()
{
   if ( !autoShrink ) return;

/* Decrease space if necessary */

   int  needed = _count * sizeof(CallbackC*);
...
int
CallbackListC::indexOf(const CallbackC& obj) const
{
...
int
CallbackListC::indexOf(const CallbackC* op) const
{
/* Loop through the list until the entry is found */
...
CallbackC*
CallbackListC::insert(const CallbackC* op, unsigned i)
{
...
/* Otherwise, see if index is valid */
   else if (i > _count) return (CallbackC*)NULL;
...
CallbackC*
CallbackListC::replace(const CallbackC* op, unsigned i)
{
/* See if index is valid */
   if (i > _count) return (CallbackC*)NULL;

/* Don't allow same address in list twice */
   if ( op != (CallbackC*)NULL && !allow_duplicates ) {
      int	index = indexOf(op);
      if ( index != NULL_INDEX ) return _list[index];
   }
...
void
CallbackListC::remove(unsigned index)
{
   if (index < _count) {
...
void
CallbackListC::remove(const CallbackC& obj)
{
/* Look up this entry */
...
void
CallbackListC::remove(const CallbackC* op)
{
/* Look up this entry */
...
void
CallbackListC::removeNulls()
{
/* Collapse list */
   register int	j = 0;
   register int i;
   for (i=0; i<_count; i++) {
      if ( _list[i] != (CallbackC*)NULL ) {
...
CallbackC*
CallbackListC::before(const CallbackC* op) const
{
/* Look up this entry */

   register int i = indexOf(op);
   if ( i == NULL_INDEX || i == 0 ) return (CallbackC*)NULL;
   return _list[i-1];
}
...
unsigned
CallbackListC::orderOf(const CallbackC& obj) const
{
/*
...
} /* End CallbackListC::orderOf */

/*-------------------------------------------------------------*/

int
CallbackListC::compare(const void *a, const void *b)
{
   CallbackC	*ta = *(CallbackC **)a;
   CallbackC	*tb = *(CallbackC **)b;
...
CallbackListC::sort()
{
   qsort((void *)_list, (size_t)_count, (size_t)sizeof(CallbackC*),
...
void
CallbackListC::sort(int (*compar)(const void*, const void*))
{
   if ( !compar ) return;
   qsort((void *)_list, (size_t)_count, (size_t)sizeof(CallbackC*), compar);
}
