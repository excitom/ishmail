...
RowOrColListC::RowOrColListC() : GROWTH_AMOUNT(4), NULL_INDEX(-1), MAX_GROWTH(32768)
{
   _list		= (RowOrColC**)NULL;
   _count		= 0;
   _space		= 0;
...
RowOrColListC::RowOrColListC(unsigned ga) : GROWTH_AMOUNT(ga), NULL_INDEX(-1), MAX_GROWTH(ga)
{
   _list		= (RowOrColC**)NULL;
   _count		= 0;
   _space		= 0;
...
RowOrColListC::RowOrColListC(const RowOrColListC& l):GROWTH_AMOUNT(4), NULL_INDEX(-1), MAX_GROWTH(32768)
{
   _list		= (RowOrColC**)NULL;
   _count		= 0;
   _space		= 0;
...
RowOrColListC&
RowOrColListC::operator+=(const RowOrColListC& l)
{
   if ( this != &l ) {
...
unsigned
RowOrColListC::orderOf(const RowOrColC* op) const
{
   return orderOf(*op);
}
...
int
RowOrColListC::includes(const RowOrColC& obj) const
{
   return (indexOf(obj) != NULL_INDEX);
}

int
RowOrColListC::includes(const RowOrColC* op) const
{
...
RowOrColC*
RowOrColListC::append(const RowOrColC* op)
{
   return insert(op, _count);
}

RowOrColC*
RowOrColListC::prepend(const RowOrColC* op)
{
   return insert(op, 0); 
}

RowOrColC*
RowOrColListC::add(const RowOrColC* op)
{
   return append(op); 
}
...
void
RowOrColListC::SetSorted(char val)
{
   if ( sorted != val ) {
      sorted = val;
      if ( sorted ) sort();
...
void
RowOrColListC::AllowDuplicates(char val)
{
   allow_duplicates = val;
}

void
RowOrColListC::AutoShrink(char val)
{
   autoShrink = val;
   if ( autoShrink ) shrink();
}
...
void
RowOrColListC::SetCapacity(unsigned count)
{
   grow(count*sizeof(RowOrColC*));
}

RowOrColC*
RowOrColListC::operator[](unsigned i) const
{
   RowOrColC	*tp = (RowOrColC*)NULL;
   if (i < _count)
      tp = _list[i];
   else {
      cerr << "RowOrColListC[" << i << "]: reference out of bounds.";
...
ostream&
RowOrColListC::printOn(ostream& strm) const
{
...
void
RowOrColListC::grow(unsigned newSize)
{
/* Increase space */
...
/* Allocate memory for new list */
   RowOrColC	**old_list = _list;
   _list = new RowOrColC*[_space];
   if ( !_list ) {
...
void
RowOrColListC::shrink()
{
   if ( !autoShrink ) return;

/* Decrease space if necessary */

   int  needed = _count * sizeof(RowOrColC*);
...
int
RowOrColListC::indexOf(const RowOrColC& obj) const
{
...
int
RowOrColListC::indexOf(const RowOrColC* op) const
{
/* Loop through the list until the entry is found */
...
RowOrColC*
RowOrColListC::insert(const RowOrColC* op, unsigned i)
{
...
/* Otherwise, see if index is valid */
   else if (i > _count) return (RowOrColC*)NULL;
...
RowOrColC*
RowOrColListC::replace(const RowOrColC* op, unsigned i)
{
/* See if index is valid */
   if (i > _count) return (RowOrColC*)NULL;

/* Don't allow same address in list twice */
   if ( op != (RowOrColC*)NULL && !allow_duplicates ) {
      int	index = indexOf(op);
      if ( index != NULL_INDEX ) return _list[index];
   }
...
void
RowOrColListC::remove(unsigned index)
{
   if (index < _count) {
...
void
RowOrColListC::remove(const RowOrColC& obj)
{
/* Look up this entry */
...
void
RowOrColListC::remove(const RowOrColC* op)
{
/* Look up this entry */
...
void
RowOrColListC::removeNulls()
{
/* Collapse list */
   register int	j = 0;
   register int i;
   for (i=0; i<_count; i++) {
      if ( _list[i] != (RowOrColC*)NULL ) {
...
RowOrColC*
RowOrColListC::before(const RowOrColC* op) const
{
/* Look up this entry */

   register int i = indexOf(op);
   if ( i == NULL_INDEX || i == 0 ) return (RowOrColC*)NULL;
   return _list[i-1];
}
...
unsigned
RowOrColListC::orderOf(const RowOrColC& obj) const
{
/*
...
} /* End RowOrColListC::orderOf */

/*-------------------------------------------------------------*/

int
RowOrColListC::compare(const void *a, const void *b)
{
   RowOrColC	*ta = *(RowOrColC **)a;
   RowOrColC	*tb = *(RowOrColC **)b;
...
RowOrColListC::sort()
{
   qsort((void *)_list, (size_t)_count, (size_t)sizeof(RowOrColC*),
...
void
RowOrColListC::sort(int (*compar)(const void*, const void*))
{
   if ( !compar ) return;
   qsort((void *)_list, (size_t)_count, (size_t)sizeof(RowOrColC*), compar);
}
