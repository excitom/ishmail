/*
 * $Id: SmIconViewC.C,v 1.2 2000/05/07 12:26:11 fnevgeny Exp $
 *
...
char *
SmIconViewC::ButtonName() const
{
   return "smallIconViewPB";
}
...
void
SmIconViewC::GetPixmap(IconDataC& data)
{
   PixmapC	*pm;
...
void
SmIconViewC::DrawItem(const VItemC& item, const IconDataC& data,
...
//
   if ( data.pixmap ) {
	 ...
	 x = data.bounds.xmin + hlThick + itemMarginWd
	   - viewBox->HScrollValue();
	 y = data.bounds.ymin + ((data.bounds.ht - pm->ht) / 2)
...
void
SmIconViewC::DrawLabel(const IconDataC& data, Drawable drawto)
{
//
// If drawto is NULL, draw to both the offscreen pixmap and the visible window
//
   ...
   int	y = data.bounds.ymin + ((data.bounds.ht - data.labelHt) / 2)
	  - viewBox->VScrollValue();
   int	x = data.bounds.xmin + itemMarginWd + maxPmWd + labelOffset
	  - viewBox->HScrollValue();
   GC	gc = viewBox->ViewGC();
...
//
	 ...
	 XmStringDraw(halApp->display, dst1, fontList, (XmString)wstr,
		      gc, x, y, ldata->width, XmALIGNMENT_BEGINNING,  
...
void
SmIconViewC::UpdateMaxItemWd()
{
   maxItemWd = maxPmWd + maxLabelWd + labelOffset + (hlThick+itemMarginWd)*2;
}

void
SmIconViewC::UpdateMaxItemHt()
{
   maxItemHt = MAX(maxPmHt, maxLabelHt) + (hlThick+itemMarginHt)*2;
}
