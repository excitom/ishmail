...
RowColChildListC::RowColChildListC() : GROWTH_AMOUNT(4), NULL_INDEX(-1), MAX_GROWTH(32768)
{
   _list		= (RowColChildC**)NULL;
   _count		= 0;
   _space		= 0;
...
RowColChildListC::RowColChildListC(unsigned ga) : GROWTH_AMOUNT(ga), NULL_INDEX(-1), MAX_GROWTH(ga)
{
   _list		= (RowColChildC**)NULL;
   _count		= 0;
   _space		= 0;
...
RowColChildListC::RowColChildListC(const RowColChildListC& l):GROWTH_AMOUNT(4), NULL_INDEX(-1), MAX_GROWTH(32768)
{
   _list		= (RowColChildC**)NULL;
   _count		= 0;
   _space		= 0;
...
RowColChildListC::~RowColChildListC()
{
...
RowColChildListC&
RowColChildListC::operator+=(const RowColChildListC& l)
{
   if ( this != &l ) {
...
unsigned
RowColChildListC::orderOf(const RowColChildC* op) const
{
   return orderOf(*op);
}
...
int
RowColChildListC::includes(const RowColChildC& obj) const
{
   return (indexOf(obj) != NULL_INDEX);
}

int
RowColChildListC::includes(const RowColChildC* op) const
{
...
RowColChildC*
RowColChildListC::append(const RowColChildC* op)
{
   return insert(op, _count);
}

RowColChildC*
RowColChildListC::prepend(const RowColChildC* op)
{
   return insert(op, 0); 
}

RowColChildC*
RowColChildListC::add(const RowColChildC* op)
{
   return append(op); 
}
...
void
RowColChildListC::SetSorted(char val)
{
   if ( sorted != val ) {
      sorted = val;
      if ( sorted ) sort();
...
void
RowColChildListC::AllowDuplicates(char val)
{
   allow_duplicates = val;
}

void
RowColChildListC::AutoShrink(char val)
{
   autoShrink = val;
   if ( autoShrink ) shrink();
}
...
void
RowColChildListC::SetCapacity(unsigned count)
{
   grow(count*sizeof(RowColChildC*));
}

RowColChildC*
RowColChildListC::operator[](unsigned i) const
{
   RowColChildC	*tp = (RowColChildC*)NULL;
   if (i < _count)
      tp = _list[i];
   else {
      cerr << "RowColChildListC[" << i << "]: reference out of bounds.";
...
ostream&
RowColChildListC::printOn(ostream& strm) const
{
...
void
RowColChildListC::grow(unsigned newSize)
{
/* Increase space */
...
/* Allocate memory for new list */
   RowColChildC	**old_list = _list;
   _list = new RowColChildC*[_space];
   if ( !_list ) {
...
void
RowColChildListC::shrink()
{
   if ( !autoShrink ) return;

/* Decrease space if necessary */

   int  needed = _count * sizeof(RowColChildC*);
...
int
RowColChildListC::indexOf(const RowColChildC& obj) const
{
...
int
RowColChildListC::indexOf(const RowColChildC* op) const
{
/* Loop through the list until the entry is found */
...
RowColChildC*
RowColChildListC::insert(const RowColChildC* op, unsigned i)
{
...
/* Otherwise, see if index is valid */
   else if (i > _count) return (RowColChildC*)NULL;
...
RowColChildC*
RowColChildListC::replace(const RowColChildC* op, unsigned i)
{
/* See if index is valid */
   if (i > _count) return (RowColChildC*)NULL;

/* Don't allow same address in list twice */
   if ( op != (RowColChildC*)NULL && !allow_duplicates ) {
      int	index = indexOf(op);
      if ( index != NULL_INDEX ) return _list[index];
   }
...
void
RowColChildListC::remove(unsigned index)
{
   if (index < _count) {
...
void
RowColChildListC::remove(const RowColChildC& obj)
{
/* Look up this entry */
...
void
RowColChildListC::remove(const RowColChildC* op)
{
/* Look up this entry */
...
void
RowColChildListC::removeNulls()
{
/* Collapse list */
   register int	j = 0;
   register int i;
   for (i=0; i<_count; i++) {
      if ( _list[i] != (RowColChildC*)NULL ) {
...
RowColChildC*
RowColChildListC::before(const RowColChildC* op) const
{
/* Look up this entry */

   register int i = indexOf(op);
   if ( i == NULL_INDEX || i == 0 ) return (RowColChildC*)NULL;
   return _list[i-1];
}
...
unsigned
RowColChildListC::orderOf(const RowColChildC& obj) const
{
/*
...
} /* End RowColChildListC::orderOf */

/*-------------------------------------------------------------*/

int
RowColChildListC::compare(const void *a, const void *b)
{
   RowColChildC	*ta = *(RowColChildC **)a;
   RowColChildC	*tb = *(RowColChildC **)b;
...
void
RowColChildListC::sort()
{
   qsort((void *)_list, (size_t)_count, (size_t)sizeof(RowColChildC*),
...
void
RowColChildListC::sort(int (*compar)(const void*, const void*))
{
   if ( !compar ) return;
   qsort((void *)_list, (size_t)_count, (size_t)sizeof(RowColChildC*), compar);
}
