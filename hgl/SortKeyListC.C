...
SortKeyListC::SortKeyListC() : GROWTH_AMOUNT(4), NULL_INDEX(-1), MAX_GROWTH(32768)
{
   _list		= (SortKeyC**)NULL;
   _count		= 0;
   _space		= 0;
...
SortKeyListC::SortKeyListC(unsigned ga) : GROWTH_AMOUNT(ga), NULL_INDEX(-1), MAX_GROWTH(ga)
{
   _list		= (SortKeyC**)NULL;
   _count		= 0;
   _space		= 0;
...
SortKeyListC::SortKeyListC(const SortKeyListC& l):GROWTH_AMOUNT(4), NULL_INDEX(-1), MAX_GROWTH(32768)
{
   _list		= (SortKeyC**)NULL;
   _count		= 0;
   _space		= 0;
...
SortKeyListC&
SortKeyListC::operator+=(const SortKeyListC& l)
{
   if ( this != &l ) {
...
unsigned
SortKeyListC::orderOf(const SortKeyC* op) const
{
   return orderOf(*op);
}
...
int
SortKeyListC::includes(const SortKeyC& obj) const
{
   return (indexOf(obj) != NULL_INDEX);
}

int
SortKeyListC::includes(const SortKeyC* op) const
{
...
SortKeyC*
SortKeyListC::append(const SortKeyC* op)
{
   return insert(op, _count);
}

SortKeyC*
SortKeyListC::prepend(const SortKeyC* op)
{
   return insert(op, 0); 
}

SortKeyC*
SortKeyListC::add(const SortKeyC* op)
{
   return append(op); 
}
...
void
SortKeyListC::SetSorted(char val)
{
   if ( sorted != val ) {
      sorted = val;
      if ( sorted ) sort();
...
void
SortKeyListC::AllowDuplicates(char val)
{
   allow_duplicates = val;
}

void
SortKeyListC::AutoShrink(char val)
{
   autoShrink = val;
   if ( autoShrink ) shrink();
}
...
void
SortKeyListC::SetCapacity(unsigned count)
{
   grow(count*sizeof(SortKeyC*));
}

SortKeyC*
SortKeyListC::operator[](unsigned i) const
{
   SortKeyC	*tp = (SortKeyC*)NULL;
   if (i < _count)
      tp = _list[i];
   else {
      cerr << "SortKeyListC[" << i << "]: reference out of bounds.";
...
ostream&
SortKeyListC::printOn(ostream& strm) const
{
...
void
SortKeyListC::grow(unsigned newSize)
{
/* Increase space */
...
/* Allocate memory for new list */
   SortKeyC	**old_list = _list;
   _list = new SortKeyC*[_space];
   if ( !_list ) {
...
/* Decrease space if necessary */
   ...
   int  needed = _count * sizeof(SortKeyC*);
...
int
SortKeyListC::indexOf(const SortKeyC& obj) const
{
...
int
SortKeyListC::indexOf(const SortKeyC* op) const
{
/* Loop through the list until the entry is found */
...
SortKeyC*
SortKeyListC::insert(const SortKeyC* op, unsigned i)
{
...
/* Otherwise, see if index is valid */
   else if (i > _count) return (SortKeyC*)NULL;
...
SortKeyC*
SortKeyListC::replace(const SortKeyC* op, unsigned i)
{
/* See if index is valid */
   if (i > _count) return (SortKeyC*)NULL;

/* Don't allow same address in list twice */
   if ( op != (SortKeyC*)NULL && !allow_duplicates ) {
      int	index = indexOf(op);
      if ( index != NULL_INDEX ) return _list[index];
   }
...
void
SortKeyListC::remove(unsigned index)
{
   if (index < _count) {
...
void
SortKeyListC::remove(const SortKeyC& obj)
{
/* Look up this entry */
...
void
SortKeyListC::remove(const SortKeyC* op)
{
/* Look up this entry */
...
void
SortKeyListC::removeNulls()
{
/* Collapse list */
   register int	j = 0;
   register int i;
   for (i=0; i<_count; i++) {
      if ( _list[i] != (SortKeyC*)NULL ) {
...
SortKeyC*
SortKeyListC::before(const SortKeyC* op) const
{
/* Look up this entry */

   register int i = indexOf(op);
   if ( i == NULL_INDEX || i == 0 ) return (SortKeyC*)NULL;
   return _list[i-1];
}
...
unsigned
SortKeyListC::orderOf(const SortKeyC& obj) const
{
/*
...
} /* End SortKeyListC::orderOf */

/*-------------------------------------------------------------*/

int
SortKeyListC::compare(const void *a, const void *b)
{
   SortKeyC	*ta = *(SortKeyC **)a;
   SortKeyC	*tb = *(SortKeyC **)b;
...
SortKeyListC::sort()
{
   qsort((void *)_list, (size_t)_count, (size_t)sizeof(SortKeyC*),
...
void
SortKeyListC::sort(int (*compar)(const void*, const void*))
{
   if ( !compar ) return;
   qsort((void *)_list, (size_t)_count, (size_t)sizeof(SortKeyC*), compar);
}
